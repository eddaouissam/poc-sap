...
FROM
  sap_silver.s_mbew AS mbew
LEFT JOIN
  sap_silver.s_mbewh AS mbewh
  ON
...
LEFT JOIN
  sap_silver.s_t001k AS t001k
  ON mbew.MANDT = t001k.MANDT
    AND mbew.BWKEY = t001k.BWKEY
LEFT JOIN
  sap_silver.s_t001 AS t001
...
--##CORTEX-CUSTOMER If you prefer to use currency conversion, uncomment below
-- LEFT JOIN sap_gold.currency_conversion AS currency_conversion
...
LEFT JOIN utils.calendar_date_dim AS CalendarDateDimension_LAEPR
  ON CalendarDateDimension_LAEPR.Date = mbew.LAEPR
LEFT JOIN utils.calendar_date_dim AS CalendarDateDimension_ZKDAT
