...
FROM sap_gold.SalesOrders AS SalesOrders
LEFT JOIN sap_gold.Deliveries AS Deliveries
  ON
...
LEFT JOIN
  sap_gold.DistributionChannelsMD AS DistributionChannelMD
  ON
...
LEFT JOIN
  sap_gold.CountriesMD AS CountriesMD
  ON
 ...
 -- if not using the view
LEFT JOIN
  sap_gold.OneTouchOrder AS OneTouchOrder
  ON
...
LEFT JOIN
  sap_gold.DivisionsMD AS DivisionsMD
...
LEFT JOIN sap_gold.DeliveryBlockingReasonsMD AS TVLST
  ON
...
LEFT JOIN sap_gold.BillingBlockingReasonsMD AS TVFST
  ON
