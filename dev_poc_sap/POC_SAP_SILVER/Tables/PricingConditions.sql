...
LEFT JOIN
  sap_gold.currency_decimal AS currency_decimal
  ON
    COALESCE(konv.WAERS, '') = currency_decimal.CURRKEY
--##CORTEX-CUSTOMER If you prefer to use currency conversion, uncomment below
-- LEFT JOIN
--   sap_gold.currency_conversion AS currency_conversion
-- ON
...
LEFT JOIN
  utils.calendar_date_dim AS CalendarDateDimension_KDATU
  ON
